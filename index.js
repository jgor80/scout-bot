...
async function createScoutingReport(clubName) {
  ...
  // 1) Try to find the club on ANY platform in PLATFORMS order
  let found = null;
  ...
  for (const platform of PLATFORMS) {
    try {
      ...
      console.warn(`⚠️ Lookup failed for ${clubName} on ${platform}:`, err.message || err);
    }
  }
  ...
  // 2) Fetch core club data + stats + recent matches on that platform
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      // /scoutclub – EA Pro Clubs scouting report (search all platforms)
      if (cmd === 'scoutclub') {
